...
const WORKBOOK_PATH = path.resolve(PROJECT_ROOT, 'files (1)', 'WEEKLY-DCS-angelica.xlsx');
const TARGET_COLLECTOR_NAME = 'Angelica Polo';
...
function isBorrowerRow(row, sheetName) {
  ...
  const collectorCell = displayName(row[3]);
  return collectorCell.toLowerCase().includes('angelica');
}
...
async function main() {
  ...
  try {
    ...
    console.log('Angelica borrower assignment');
